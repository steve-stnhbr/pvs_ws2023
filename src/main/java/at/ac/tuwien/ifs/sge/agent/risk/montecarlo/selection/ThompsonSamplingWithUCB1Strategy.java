...
public class ThompsonSamplingWithUCB1Strategy extends MCTSSelectionStrategy<Risk, RiskAction> {
    ...
    public MCTSNode<Risk, RiskAction> select(MCTSNode<Risk, RiskAction> root, MCTSTree<Risk, RiskAction> tree) {
        ...
        if (nodesForState == null || nodesForState.isEmpty()) {
            UCB1SelectionStrategy<Risk, RiskAction> ucb1 = new UCB1SelectionStrategy<>();
            return ucb1.select(root, tree);
        } else {
